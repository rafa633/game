...
// ➗ Geração de divisões exatas
function gerarFasesDivisao(qtd) {
  const fases = [];
  for (let i = 0; i < qtd; i++) {
    const b = Math.floor(Math.random() * 9) + 1; // divisor entre 1 e 9
    const resultado = Math.floor(Math.random() * 10) + 1; // resultado entre 1 e 10
    const a = b * resultado; // garante divisão exata
    fases.push({ a, b });
  }
  return fases;
}
...
// 🎯 Variáveis do jogo
const todasAsFases = gerarFasesDivisao(10);
let faseAtual = 0;
let acertos = 0;
...
function mostrarFinal() {
  ...
  btnReiniciar.onclick = () => {
    const novasFases = gerarFasesDivisao(10);
