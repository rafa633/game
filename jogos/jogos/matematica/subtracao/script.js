...
// 🧮 Geração de subtrações simples
function gerarFasesSubtracao(qtd) {
  const fases = [];
  for (let i = 0; i < qtd; i++) {
    const a = Math.floor(Math.random() * 10) + 5; // garante que a >= b
...
// 🎯 Variáveis do jogo
const todasAsFases = gerarFasesSubtracao(10);
let faseAtual = 0;
let acertos = 0;

// 🎯 Elementos do jogo
const titulo = document.createElement('h1');
titulo.innerText = 'Resolva a subtração';
...
function mostrarFinal() {
  ...
  btnReiniciar.onclick = () => {
    const novasFases = gerarFasesSubtracao(10);
  ...
  const btnFechar = document.createElement
